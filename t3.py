...
class Turbojet(pyc.Cycle):
    ...
    def setup(self):
        ...
        USE_TABULAR = True #change to use either air fuel ratio or jp-a fuel
        ...
        if design:
            ...
            balance.add_balance('Nmech', val=100000.0, units='rpm', lower=500., eq_units='hp', rhs_val=0.)
            self.connect('balance.Nmech', 'Nmech')
            self.connect('shaft.pwr_net', 'balance.lhs:Nmech')

            balance.add_balance('W', val=3.0, units='lbm/s', eq_units='inch**2') #origional was 50
            self.connect('balance.W', 'inlet.Fl_I:stat:W')
...
def viewer(prob, pt, file=sys.stdout):
    ...
    summary_data = (
        ...
        prob[pt + '.perf.OPR'],
        prob.get_val(pt + '.perf.TSFC', units='kg/N/s')  # optional
    )
...
class MPTurbojet(pyc.MPCycle):
    ...
    def setup(self):
        ...
        self.set_input_defaults('DESIGN.Nmech', 100000.0, units='rpm')
...
def run_engine(design_inputs=None,od_inputs=None):
    ...
    if od_inputs is None:
        od_inputs = [
            {"mach": 0.05, "alt": 0, "T4": 2200},
            {"mach": 0.075, "alt": 200, "T4": 2200},
            {"mach": 0.10, "alt": 400, "T4": 2200},
        ]

    prob = om.Problem()

    mp_turbojet = MPTurbojet(

        od_inputs=od_inputs

    )

    prob.model = mp_turbojet
    ...
    prob.set_val('DESIGN.fc.MN',
                 design_inputs["mach"])

    prob.set_val('DESIGN.balance.Fn_target',
                 design_inputs["Fn"],
                 units='lbf')

    prob.set_val('DESIGN.balance.T4_target',
                 design_inputs["T4"],
                 units='degR')

    prob.set_val('DESIGN.comp.PR',
                 design_inputs["PR"])

    prob.set_val('DESIGN.comp.eff',
                 design_inputs["comp_eff"])

    prob.set_val('DESIGN.turb.eff',
                 design_inputs["turb_eff"])

    # Set initial guesses for balances for DESGIN PTS
    prob['DESIGN.balance.FAR'] = 0.0175506829934
    prob['DESIGN.balance.W'] = 3.3
    prob['DESIGN.balance.turb_PR'] = 4.46138725662
    prob['DESIGN.fc.balance.Pt'] = 14.6955113159
    prob['DESIGN.fc.balance.Tt'] = 518.665288153

    # Set initial guesses for balances for OFF-DESGIN PTS
    for i,pt in enumerate(mp_turbojet.od_pts):

        # initial guesses
        prob[pt+'.balance.W'] = 3.0 #original was 50
        prob[pt+'.balance.FAR'] = 0.01680
        prob[pt+'.balance.Nmech'] = 100000.0
    ...
    #
    # print("Corrected Airflow (Wc):",
    #       prob.get_val("DESIGN.comp.Wc", units="lbm/s"))
    #
    # print("Corrected Speed (Nc):",
    #       prob.get_val("DESIGN.comp.Nc", units="rpm"))
    #
    # print("Scale Wc (s_Wc):",
    #       prob.get_val("DESIGN.comp.s_Wc"))
    #
    # print("Scale PR (s_PR):",
    #       prob.get_val("DESIGN.comp.s_PR"))
    #
    # print("Scale Eff (s_eff):",
    #       prob.get_val("DESIGN.comp.s_eff"))
    #
    # print("Scale Nc (s_Nc):",
    #       prob.get_val("DESIGN.comp.s_Nc"))
    #
    # print("\n================ TURBINE SCALING ================\n")
    #
    # print("Corrected Flow:",
    #       prob.get_val("DESIGN.turb.Wp", units="lbm/s"))
    #
    # print("Scale Flow:",
    #       prob.get_val("DESIGN.turb.s_Wp"))
    #
    # print("Scale PR:",
    #       prob.get_val("DESIGN.turb.s_PR"))
    #
    # print("Scale Eff:",
    #       prob.get_val("DESIGN.turb.s_eff"))
    #
    # print("Scale Speed:",
    #       prob.get_val("DESIGN.turb.s_Np"))
    #
    # print("\n================ NOZZLE ================\n")
    #
    # print("Throat Area:",
    #       prob.get_val("DESIGN.nozz.Throat:stat:area",
    #                    units="inch**2"))
    #
    # print("Exit Area:",
    #       prob.get_val("DESIGN.nozz.Fl_O:stat:area",
    #                    units="inch**2"))
    #
    # print("\n================ ENGINE SIZE ================\n")
    #
    # print("Mass Flow:",
    #       prob.get_val("DESIGN.inlet.Fl_O:stat:W",
    #                    units="kg/s"))
    #
    # print("Net Thrust:",
    #       prob.get_val("DESIGN.perf.Fn",
    #                    units="kN"))
    #
    # print("Fuel Flow:",
    #       prob.get_val("DESIGN.burner.Wfuel",
    #                    units="kg/s"))
    ...
    return prob, mp_turbojet.od_pts
